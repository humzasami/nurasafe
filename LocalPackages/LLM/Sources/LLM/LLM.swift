...
public actor LLMCore {
    ...
    /// Prompt length in tokens after `prepareContext` — used to cap *new* tokens only.
    private var promptLengthTokens: Int32 = 0
    /// When set, stop after this many generated tokens (independent of context window size).
    private var maxNewTokens: Int32?
    /// > 1.0 enables `llama_sampler_init_penalties` (reduces repetition loops). 1.0 = off.
    private var repeatPenalty: Float = 1.0
    
    public func setParameters(seed: UInt32? = nil, topK: Int32? = nil, topP: Float? = nil, temp: Float? = nil) {
    ...
    /// Caps how many tokens the model may generate for the next `generateResponseStream`. Pass `nil` for no cap.
    public func setMaxNewTokens(_ limit: Int32?) {
        maxNewTokens = limit.map { max(1, $0) }
    }
    
    public func setRepeatPenalty(_ penalty: Float) {
        repeatPenalty = max(1.0, penalty)
    }
    
    public func setStopSequence(_ sequence: String?) {
        if let sequence {
    ...
    func prepareContext(for input: String) -> Bool {
        ...
        promptLengthTokens = Int32(initialCount)
    ...
    func predictNextToken() -> Token {
        ...
        let generatedSoFar = currentTokenCount - promptLengthTokens
        if let cap = maxNewTokens, generatedSoFar >= cap {
        ...
        if repeatPenalty > 1.0001 {
            llama_sampler_chain_add(sampler, llama_sampler_init_penalties(128, repeatPenalty, 0.0, 0.0))
        }
    ...
    /// Clears the KV cache so the next generateResponseStream call starts fresh.
    /// Must be called before each generation to prevent llama_decode failures on 2nd+ messages.
    ...
    public func getEmbeddings(from input: String) throws -> [Float] {
